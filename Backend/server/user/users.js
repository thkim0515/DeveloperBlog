...
loadSecrets().then((secrets) => {
  ...
  const SESSION_KEY = secrets.REACT_APP_SECURECODE;
  ...
  router.post("/findPwd", async (req, res) => {
    ...
    try {
      ...
      if (emailSent) {
        res.json({
          message: "Password가 이메일로 전송되었습니다.",
          id: user.id,
        });
      } else {
        res.status(500).json({ message: "ID 이메일 전송 실패" });
  ...
  router.put("/update/:_id", async (req, res) => {
    try {
      ...
      logError("유저 업데이트", error.message);
      res.status(500).json({ message: "서버 에러" });
    }
  });
  ...
  router.put("/updatePwd/:_id", async (req, res) => {
    try {
      ...
      logError("유저 패스워드 업데이트", error.message);
      res.status(500).json({ message: "서버 에러" });
    }
  });
